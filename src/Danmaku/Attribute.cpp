...
QFont Attribute::getFont(int pixelSize, QString family)
{
	QFont font;
	font.setBold(Setting::getValue("/Danmaku/Effect", 5) % 2);
	font.setFamily(family);
	font.setPixelSize(pixelSize);
	return font;
}

QSize  Attribute::getSize(QString string, QFont font)
{
...
QSizeF Attribute::getPlayer(qint64 date)
{
	return date <= 1384099200 ? QSizeF(545, 388) : QSizeF(862, 568);
}

double  Attribute::getScale(int mode, qint64 date, QSize size)
{
	int m = Setting::getValue("/Danmaku/Scale/Fitted", 0x1);
	if (mode == 7 && (m & 0x1) == 0)
	{
		return 0;
	}
	if (mode <= 6 && (m & 0x2) == 0)
	{
		return Setting::getValue("/Danmaku/Scale/Factor", 1.0);
	}
...
QImage  Attribute::getCache(QString string,
	int color,
	QFont font,
	QSize size,
	bool frame,
